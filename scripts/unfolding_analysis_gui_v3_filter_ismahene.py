...
try:
    # Add the neural networks path to sys.path
    import sys
    from pathlib import Path
    nn_path = Path(__file__).parent.parent / 'src'
    ...
    NN_AVAILABLE = True
    print("✓ Neural Network modules loaded successfully")
except ImportError as e:
    NN_AVAILABLE = False
    print(f"⚠️ Neural Network modules not available: {e}")
...
class TetherAnalysisGUI(QMainWindow):
    def __init__(self, root_dir, *args, **kwargs):
        ...
        # Initialize neural network analysis (SINGLE INITIALIZATION)
        ...
        if NN_AVAILABLE:
            ...
            print("⚠️ Neural Network modules not available - NN features disabled")
            self.nn_enabled = False
            self.nn_analysis = None
    ...
    def setupUI(self):
        ...
        # Neural Network plateau detection checkbox
        self.nn_plateaus_check = QCheckBox("NN Plateaus")
        self.nn_plateaus_check.setChecked(self.nn_enabled)
        self.nn_plateaus_check.setEnabled(self.nn_enabled)
        if self.nn_enabled:
            self.nn_plateaus_check.setToolTip("Show neural network detected plateaus")
            self.nn_plateaus_check.setStyleSheet("QCheckBox { color: #FF6B35; font-weight: bold; }")
        else:
            self.nn_plateaus_check.setToolTip("Neural network modules not available")
            self.nn_plateaus_check.setStyleSheet("QCheckBox { color: #888888; }")
        
        print(f"🔍 DEBUG: NN checkbox created - enabled: {self.nn_enabled}, NN_AVAILABLE: {NN_AVAILABLE}, analysis obj: {self.nn_analysis is not None}")
        ...
        plot_controls.addWidget(self.nn_plateaus_check)
    ...
    # Removed interactive threshold drag handler
        ...
        default_instructions = """Welcome to Tether Analysis GUI with Neural Networks!
...
• Toggle "NN Plateaus" checkbox to show/hide neural network results
• NN plateaus displayed in cyan with confidence scores
• Complements traditional plateau detection methods

🚀 Performance: Concurrent processing uses all CPU cores for 5-6x speedup!
Ready to analyze TDMS files efficiently with AI assistance!"""
        ...
        self.nn_plateaus_check.stateChanged.connect(self.update_plot_display)
    ...
    def setupMenuBar(self):
        ...
        # Toggle NN plateaus
        toggle_nn_plateaus_action = QAction('Toggle Neural Network Plateaus', self)
        toggle_nn_plateaus_action.setCheckable(True)
        toggle_nn_plateaus_action.setChecked(self.nn_plateaus_check.isChecked())
        toggle_nn_plateaus_action.setEnabled(self.nn_enabled)
        toggle_nn_plateaus_action.triggered.connect(lambda checked: self.nn_plateaus_check.setChecked(checked))
        if self.nn_enabled:
            toggle_nn_plateaus_action.setStatusTip('Toggle neural network detected plateaus')
        else:
            toggle_nn_plateaus_action.setStatusTip('Neural network modules not available')
        view_menu.addAction(toggle_nn_plateaus_action)
...
• Typical accuracy: ~85% (varies by dataset)"""
    ...
    def file_prev(self):
        """Navigate to previous file in current table order (respects sorting)"""
    ...
    def run_analysis(self):
        ...
        try:
            ...
            # Run neural network plateau detection if enabled
            if self.nn_enabled and self.nn_analysis:
                try:
                    ...
                    if len(force_data) > 0 and len(time_data) > 0:
                        ...
                        print(f"Neural network detected {nn_count} plateaus")
                    else:
    ...
    def update_plot_with_analysis(self, result):
        ...
        try:
            ...
            # Plot neural network detected plateaus
            if self.nn_plateaus_check.isChecked() and result.get('nn_plateaus'):
                rel_time = result['rel_time']
                defl_savitz = result['defl_savitz']
                nn_plateaus = result['nn_plateaus']
                
                # Get appropriate x-axis data (time or displacement)
                x_data = self.get_x_axis_data(result, rel_time)
                
                # Plot NN plateaus with distinct styling
                for i, plateau in enumerate(nn_plateaus):
                    start_time = plateau['start_time']
                    end_time = plateau['end_time']
                    confidence = plateau['confidence']
                    
                    # Find indices closest to the time boundaries
                    start_idx = np.argmin(np.abs(rel_time - start_time))
                    end_idx = np.argmin(np.abs(rel_time - end_time))
                    
                    if start_idx < end_idx and end_idx < len(defl_savitz):
                        # NN plateau region with distinctive styling
                        self.plotview.plot(x_data[start_idx:end_idx], defl_savitz[start_idx:end_idx],
                                         pen=pg.mkPen(color='cyan', width=4, style=Qt.DotLine),
                                         name=f'NN Plateau {i+1} ({confidence:.2f})')
                        
                        # NN average line
                        plateau_avg = plateau['avg_force']
                        self.plotview.plot([x_data[start_idx], x_data[end_idx-1]],
                                         [plateau_avg, plateau_avg],
                                         pen=pg.mkPen(color='cyan', width=3, style=Qt.DashDotLine),
                                         name=f'NN Avg {i+1}')
                        
                        # NN plateau markers (distinctive)
                        center_idx = (start_idx + end_idx) // 2
                        self.plotview.plot([x_data[center_idx]], [defl_savitz[center_idx]],
                                         pen=None, symbol='d', symbolSize=10,  # Diamond symbol
                                         symbolBrush='cyan', symbolPen='darkcyan',
                                         name=f'NN Center {i+1}')
    ...
    def update_plot_display(self):
        ...
        print(f"🔍 DEBUG: update_plot_display called - NN checkbox checked: {self.nn_plateaus_check.isChecked()}")
        if hasattr(self, 'current_analysis_result') and self.current_analysis_result:
    ...
    def update_results_display(self, result):
        ...
        try:
            ...
            results_text += f"Traditional plateaus found: {len(result['plateaus'])}\n"
            
            # Add neural network results if available
            if result.get('nn_plateaus'):
                nn_count = len(result['nn_plateaus'])
                results_text += f"Neural Network plateaus: {nn_count}\n"
                
                # Show NN plateau details
                for i, plateau in enumerate(result['nn_plateaus']):
                    confidence = plateau['confidence']
                    length = plateau['length']
                    results_text += f"  NN Plateau {i+1}: confidence={confidence:.3f}, length={length:.2f}s\n"
            elif self.nn_enabled:
                results_text += "Neural Network plateaus: 0\n"
...
All file-specific parameters have been preserved."""
    ...
    def run_concurrent_batch_analysis(self):
        ...
        # For session files, disable auto-labeling to preserve existing good/bad labels
        # For new files without labels, we could potentially use auto-labeling
        auto_label = False   # Always disable auto-labeling to preserve session labels
