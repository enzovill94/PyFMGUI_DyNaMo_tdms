...
class TetherAnalysisGUI(QMainWindow):
    ...
    def setupUI(self):
        ...
        files_label = QLabel("Force Curve Files (Drag & Drop TDMS/JPK folders/files here)")
...
• This will load a directory and analyze all TDMS/JPK files concurrently
• Alternatively: Drag folders or TDMS/JPK files into the file table
...
Ready to analyze TDMS and JPK force curve files efficiently!"""
    ...
    def setupMenuBar(self):
        ...
        change_dir_action.setStatusTip('Change the root directory for TDMS/JPK force curve files')
        ...
        batch_analysis_action.setStatusTip('Load directory and analyze all TDMS/JPK files with concurrent processing (main portal)')
    ...
    def change_directory(self):
        """Change the root directory for TDMS/JPK force curve files"""
        try:
            # Open directory selection dialog
            new_directory = QFileDialog.getExistingDirectory(
                self,
                "Select Directory with TDMS or JPK Force Files",
                self.root_dir,
            ...
            if new_directory:
                ...
                # Update status
                if self.file_path:
                    self.status_label.setText(f"Loaded {len(self.file_path)} force curve files from selected directory")
                else:
                    self.status_label.setText("No TDMS or JPK files found in selected directory - try drag-and-drop")
                self.results_text.setText(f"Directory changed from:\n{old_directory}\n\nTo:\n{new_directory}\n\nClick a file to start analyzing force curves.")
    ...
    def handle_dropped_files(self, dropped_paths):
        ...
        files = [path for path in dropped_paths if path.endswith('.tdms') or path.endswith('.jpk-force')]
    ...
    def find_tdms_files(self, root_dir):
        """Find all TDMS and JPK files and populate the sortable table"""
        ...
        for dirpath, dirnames, filenames in os.walk(root_dir):
            ...
            for filename in filenames:
                # Accept both .tdms and .jpk-force files
                if filename.endswith(".tdms") or filename.endswith(".jpk-force"):
    ...
    def run_batch_analysis_on_session_files(self):
        ...
        if not self.file_path:
            ...
            self.status_label.setText("Loading TDMS/JPK files from selected directory...")
            self.find_tdms_files(directory)
            self.root_dir = directory
            
            if not self.file_path:
                self.status_label.setText("No TDMS or JPK files found in selected directory")
                return
            
            self.status_label.setText(f"Loaded {len(self.file_path)} force curve files. Starting concurrent batch analysis...")
        else:
            ...
            if clicked_button == cancel_btn:
                ...
                self.status_label.setText("Loading TDMS/JPK files from new directory...")
                self.find_tdms_files(directory)
                self.root_dir = directory
                ...
                self.status_label.setText(f"Loaded {len(self.file_path)} new force curve files. Starting concurrent batch analysis...")
