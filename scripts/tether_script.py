...
def find_plateaus(x, y, params=None, dt=1e-3): 
    ...
    if len(plateaus) > 0:
        df_plat = pd.DataFrame({
            ...
            'tether_lifetime_m': [(int(((end- idx_max) * 0.9))) * dx  for start, end in plateaus], # 90 percent of plateau length
        ...
        # Create empty DataFrame with same column structure when no plateaus found
        df_plat = pd.DataFrame({
            'plateaus': [],
            'plateau_avg': [],
            'delta_avg': [],
        ...
        print("  No plateaus found - returning empty DataFrame")
