...
class TetherAnalysisGUI(QWidget):
    ...
    def setupUI(self):
        ...
        # Add derivative analysis plot above File Info & Status
        derivative_label = QLabel("Derivative Analysis (dy/dx vs Displacement)")
        derivative_label.setStyleSheet("QLabel { font-weight: bold; }")
        results_text_layout.addWidget(derivative_label)
        
        self.derivative_plotview = pg.PlotWidget()
        self.derivative_plotview.setLabel('left', 'dy/dx', units='N/m')
        self.derivative_plotview.setLabel('bottom', 'Displacement', units='m')
        self.derivative_plotview.showGrid(x=True, y=True)
        self.derivative_plotview.setMaximumHeight(200)
        self.derivative_plotview.setMinimumHeight(150)
        results_text_layout.addWidget(self.derivative_plotview)
...
Click a folder to start analyzing TDMS files."""
    ...
    # Map button indices to actions
        if button_idx == 0:
            ...
            # self.file_prev()
            print("File marked as previous")
        elif button_idx == 12:
            # self.file_next()
            print("File marked as next")
    ...
    def update_plot_with_analysis(self, result):
        ...
        try:
            ...
            self.update_derivative_plot(result)
    ...
    def update_derivative_plot(self, result):
        ...
        try:
            ...
            if 'dy_abs_sav' in df_data and 'x' in df_data:
                ...
                self.derivative_plotview.plot(x_data, dy_data,
                                             pen=pg.mkPen(color='blue', width=2),
                                             name='|dy/dx|')
                # Plot static threshold line
                self.derivative_plotview.addLine(y=pl_threshold, pen=pg.mkPen('r', width=2, style=Qt.DashLine))
        except Exception as e:
